...
import { useAddEquipmentMutation, useUpdateEquipmentMutation } from '@/services/siteManagmentApi'
...
export function AddEquipmentModal({ open, setOpen, equipment, refetch }: AddEquipmentModalProps) {
    ...
    useEffect(() => {
        if(equipment){
            setFormData({
                ...
                next_maintenance_due: equipment.next_maintenance_due ? equipment.next_maintenance_due.split('T')[0] : ''
            })
        }
        return () => {
            setFormData({
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            if(response){
                ...
                toast({
                    title: "Success",
                    description: equipment ? "Equipment updated successfully" : "Equipment added successfully",
                })
                setOpen(false)
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogContent className="flex max-h-[95vh] flex-col overflow-y-auto sm:max-w-[425px] md:min-w-[600px]">
                <DialogHeader>
                    <DialogTitle>{equipment ? "Edit Equipment" : "Add New Equipment"}</DialogTitle>
                    <DialogDescription>
                        {equipment ? "Edit details for equipment" : "Add details for new equipment to track"}
                    </DialogDescription>
                </DialogHeader>
                <form onSubmit={handleSubmit}>
                    <div className="grid gap-4 py-4">
                        ...
                        <div className="grid grid-cols-4 items-center gap-4">
                            ...
                            <Select
                                onValueChange={(value: 'available' | 'maintenance' | 'offline') => 
                    ...
                    <DialogFooter>
                        <Button type="submit" disabled={isLoading}>
                            {isLoading ? (equipment ? "Updating..." : "Adding...") : (equipment ? "Update Equipment" : "Add Equipment")}
                        </Button>
                    </DialogFooter>
                </form>
            </DialogContent>
