...
import { XIcon, AlertCircle, AlertTriangle, InfoIcon, Plus, Settings } from 'lucide-react'
...
const alertCategories: AlertCategory[] = ['inactivity', 'attendance', 'performance', 'system']
...
export default function Alerts() {
    ...
    const [alertConfigs, setAlertConfigs] = useState<Record<AlertCategory, AlertConfig>>({
    ...
    const handleAddAlert = () => {
        ...
        setNewAlert({ title: '', description: '', type: 'info', category: 'system' })
        setIsConfigOpen(false)
    }

    const handleConfigChange = (category: AlertCategory, key: string, value: boolean | number | string) => {
            ...
            <Layout.Body className='space-y-4'>
                    ...
                    <div className='flex space-x-2'>
                        <Dialog open={isConfigOpen} onOpenChange={setIsConfigOpen}>
                            <DialogTrigger asChild>
                                <Button variant="outline"><Plus className="mr-2 h-4 w-4" /> Add Alert</Button>
                            </DialogTrigger>
                            <DialogContent className="sm:max-w-[425px]">
                                ...
                                <div className="grid gap-4 py-4">
                                    ...
                                    <div className="grid grid-cols-4 items-center gap-4">
                                        ...
                                        <Select
                                            ...
                                            onValueChange={(value: AlertCategory) => setNewAlert({...newAlert, category: value})}
                                        >
                                            <SelectTrigger className="col-span-3">
                                ...
                                <DialogFooter>
                                    <Button type="submit" onClick={handleAddAlert}>Add Alert</Button>
                                </DialogFooter>
                            </DialogContent>
                        </Dialog>
                        <Dialog>
                            ...
                            <DialogContent className="sm:max-w-[425px]">
                                ...
                                <div className="grid gap-4 py-4">
                                    {(Object.entries(alertConfigs) as [AlertCategory, AlertConfig][]).map(([category, config]) => (
                                        <div key={category} className="flex items-center justify-between">
                                            ...
                                            <Switch
                                                ...
                                                onCheckedChange={(checked) => handleConfigChange(category, 'enabled', checked)}
                                            />
                                            {category === 'inactivity' && (
                                                <Input
                                                    type="number"
                                                    value={config.threshold}
                                                    onChange={(e) => handleConfigChange(category, 'threshold', parseInt(e.target.value))}
                                                    className="w-16 ml-2"
                                                    placeholder="Hours"
                                                />
                                            )}
                                            {category === 'performance' && (
                                                <Select
                                                    ...
                                                    onValueChange={(value: 'low' | 'medium' | 'high') => handleConfigChange(category, 'sensitivity', value)}
                                                >
                ...
                <div className='flex space-x-2'>
                    <Input
                        ...
                        className="max-w-sm"
                    />
                    <Select value={filter} onValueChange={(value: AlertType | 'all') => setFilter(value)}>
