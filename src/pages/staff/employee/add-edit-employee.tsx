...
export function AddEditEmployeeModal({
  ...
  const [employeeData, setEmployeeData] = useState<AddEmployeePayload>(defaultEmployeeData)
  const [departmentId, setDepartmentId] = useState<number | null>(null)
  const [roleId, setRoleId] = useState<any>()
  const [managerId, setManagerId] = useState<string>()
  const [employeeWithCredentials, setEmployeeWithCredentials] = useState<number>(0)
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name.startsWith('address.')) {
      const addressField = name.split('.')[1] as keyof AddEmployeePayload['address']
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const { address, ...employeeFields } = employeeData
      Object.entries({
        ...employeeFields,
      ...
      if (employeeData.address) {
        formData.append('address[address_line1]', employeeData.address.address_line1)
        formData.append('address[postal_code]', employeeData.address.postal_code)

        if (employeeData.address.country_id > 0) {
          formData.append('address[country_id]', employeeData.address.country_id.toString())
        }
        if (employeeData.address.state_id > 0) {
          formData.append('address[state_id]', employeeData.address.state_id.toString())
        }
        if (employeeData.address.city_id > 0) {
          formData.append('address[city_id]', employeeData.address.city_id.toString())
        }
      }
            ...
            <div className='flex-grow overflow-y-auto pr-2'>
              ...
              <div className='my-4 flex flex-col gap-4'>
                <div className='grid grid-cols-4 items-center gap-4'>
                  ...
                  <Select
                      value={employeeData.address.country_id ? employeeData.address.country_id.toString() : ''}
                      onValueChange={(value) => {
                        setEmployeeData({
                          ...employeeData,
                          address: {
                ...
                {employeeData.address.country_id > 0 && (
                    <div className='grid grid-cols-4 items-center gap-4'>
                      ...
                      <Select
                          value={employeeData.address.state_id ? employeeData.address.state_id.toString() : ''}
                          onValueChange={(value) => {
                            setEmployeeData({
                              ...employeeData,
                              address: {
                ...
                {employeeData.address.state_id > 0 && (
                    <div className='grid grid-cols-4 items-center gap-4'>
                      ...
                      <Select
                          value={employeeData.address.city_id ? employeeData.address.city_id.toString() : ''}
                          onValueChange={(value) =>
                              setEmployeeData({
                                ...employeeData,
                                address: {
