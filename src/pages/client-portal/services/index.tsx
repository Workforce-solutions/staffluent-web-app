...
export default function ClientServices() {
  ...
  const {
    ...
    isLoading,
    error,
  } = useGetClientServicesQuery({ search: debouncedSearch })

  const transformedServices = useMemo(() => {
    if (!services)
      return { active: [], pending: [], completed: [], declined: [] }
            ...
            {['active', 'pending', 'completed', 'declined'].map((tab) => (
              ...
              <TabsContent key={tab} value={tab} className='space-y-4'>
                ...
                transformedServices[tab]?.length > 0 ? (
                  // @ts-ignore
                  transformedServices[tab].map((service) => (
                    <ServiceCard
                      key={service.service_id}
                      service={service}
                      onClick={() =>
                        navigate(
                          `/client-portal/services/${service.service_id}`
                        )
                      }
                    />
                  ))
